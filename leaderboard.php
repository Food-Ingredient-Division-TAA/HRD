...
FROM daily_output WHERE dept_produk='Bumbu' AND nik_karyawan!='' AND YEAR(tgl_daily) = '20$year' GROUP BY nik_karyawan ORDER BY poin DESC LIMIT 1");
$data=mysqli_fetch_array($sql); 
$op = mysqli_query($konek,"SELECT * FROM karyawan WHERE nik='$data[nik_karyawan]'"); $kry = mysqli_fetch_array($op); 
$yretur = mysqli_query($konek,"SELECT * FROM qc_skr_komplain  LEFT JOIN `qc_skr` ON `qc_skr_komplain`.`no_skr`=`qc_skr`.`no_skr` LEFT JOIN `material` ON `qc_skr_komplain`.`kode_material`=`material`.`kode_material` WHERE material.dept_material='Bumbu' AND nik_karyawan='$data[nik_karyawan]' AND nik_karyawan!='' AND SUBSTR(kode_batch,7,2) ='$year' AND YEAR(tgl_skr) = '20$year'");
while($ydretur=mysqli_fetch_array($yretur)){  $yskr += $ydretur['qty_komplain']; }
$flv = mysqli_query($konek,"SELECT * FROM daily_output WHERE dept_produk='Bumbu' AND nik_karyawan='".$data['nik_karyawan']."' AND YEAR(tgl_daily) = '20$year' GROUP BY kode_material");
...
<nav class="navbar navbar-default" style="background:cyan; color:#000000; font-weight:bold; ">
    ...
    <span style="position:absolute; margin:auto; padding:15px; font-size:20px;">PACKING BUMBU <?php echo date('Y'); ?></span>
...
<section class="content">
  <div class="container-fluid">
    <div class="row mb-2">
      ...
      <?php $year = substr(date('Y'),2,2); $num=1;
        ...
        FROM daily_output WHERE dept_produk='Bumbu' AND nik_karyawan!='' AND YEAR(tgl_daily) = '20$year' GROUP BY nik_karyawan ORDER BY poin DESC");
        while($row=mysqli_fetch_array($query)){    $gkpcs=0;   $stdbruto=0; $sumflv=""; $yskr=0;
          $op = mysqli_query($konek,"SELECT * FROM karyawan WHERE nik='$row[nik_karyawan]'"); $kry = mysqli_fetch_array($op); 
          $yretur = mysqli_query($konek,"SELECT * FROM qc_skr_komplain  LEFT JOIN `qc_skr` ON `qc_skr_komplain`.`no_skr`=`qc_skr`.`no_skr` LEFT JOIN `material` ON `qc_skr_komplain`.`kode_material`=`material`.`kode_material` WHERE material.dept_material='Bumbu' AND nik_karyawan='$row[nik_karyawan]' AND nik_karyawan!='' AND SUBSTR(kode_batch,7,2) ='$year' AND YEAR(tgl_skr) = '20$year'");
          while($ydretur=mysqli_fetch_array($yretur)){  $yskr += $ydretur['qty_komplain']; }
          $flv = mysqli_query($konek,"SELECT * FROM daily_output WHERE dept_produk='Bumbu' AND nik_karyawan='".$row['nik_karyawan']."' AND YEAR(tgl_daily) = '20$year' GROUP BY kode_material");
          while($dflv = mysqli_fetch_array($flv)){
              $mtr = mysqli_query($konek, "SELECT * FROM `material` WHERE kode_material = '".$dflv['kode_material']."'"); $flv1 = mysqli_fetch_array($mtr);
              $digistd = sqlsrv_query($conndigi,"SELECT * FROM [idmp_palembang].[dbo].[idmp_standart] WHERE material_id = '$dflv[kode_material]'"); $datatd=sqlsrv_fetch_array($digistd, SQLSRV_FETCH_ASSOC);
              $stdbruto += number_format($datatd['std_total'],2);
